...
Простое реле времени с фиксированным интервалом срабатывания для управления, например, розеткой, в которую включено устройство, время работы которого должно быть ограничено. Например, фумигатор. Реле построено с использованием **Arduino Uno/nano/pro mini** на базе **ATmega168/328** и любого релейного модуля.
...
Реле управляется одной кнопкой. Включение реле выполняется кликом кнопки, выключение - удержанием кнопки нажатой в течение не менее 1 секунды.
...
- управление реле (switch):
  - s- переключение состояния реле;
- настройка интервала срабатывания (write):
